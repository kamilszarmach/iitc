...
window.plugin.portalcounts.getPortals = function (){
  ...
  var debugType = 0;
  var portalsToRemove = new Array();
  $.each(window.portals, function(i, portal) {
    var level = portal.options.level;
    var team = portal.options.team;
    var captured = portal.options.data.agentCaptured;
    var visited = portal.options.data.agentVisited;
    ...
    switch (team){
      case 1 :
        self.resP++;
        if (!captured) { self.resC++; }
        if (!visited) { self.resV++; }
        self.PortalsRes[level]++;
        break;
      case 2 :
        self.enlP++;
        if (!captured) { self.enlC++; }
        if (!visited) { self.enlV++; }
        self.PortalsEnl[level]++;
        break;
      default:
        self.neuP++;
        if (!captured) { self.neuC++; }
        if (!visited) { self.neuV++; }
        break;
    }
  });
  window.portalsToRemove = portalsToRemove;

  //get portals informations from IITC
  var minlvl = getMinPortalLevel();
  var total = self.neuP + self.enlP + self.resP;
  var totalC = self.neuC + self.enlC + self.resC;
  ...
  if(total > 0) {
    ...
    counts += '<tr><th>Uniques:</th><td class="enl">'+self.enlC+'</td><td class="res">'+self.resC+'</td></tr>';
    counts += '<tr><th>Unvisited:</th><td class="enl">'+self.enlV+'</td><td class="res">'+self.resV+'</td></tr>';
    ...
    counts += '</td></tr>';
      ...
      counts += '<tr><td>Unvisited neutrals:</td><td colspan="2">';
    if(minlvl > 0)
      counts += 'zoom in to see unclaimed portals';
    else
      counts += self.neuV;
    ...
    counts += '<tr><td>Uncaptured uniques:</td><td colspan="2">';
    if(minlvl > 0)
      counts += 'zoom in to see unclaimed portals';
    else
      counts += self.neuC;
